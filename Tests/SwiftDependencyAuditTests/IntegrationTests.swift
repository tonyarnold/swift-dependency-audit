...
struct IntegrationTests {
    ...
    private func parseWhitelistHelper(_ whitelist: String?) -> Set<String> {
        ...
        return Set(
            whitelist.components(separatedBy: ",").map { $0.trimmingCharacters(in: .whitespaces) }.filter {
                !$0.isEmpty
            })
    }
    ...
    private func createCompleteTestPackage() throws -> URL {
        let tempDir = FileManager.default.temporaryDirectory.appendingPathComponent(
            "CompleteTestPackage_\(UUID().uuidString)")
      ...
      try "import ArgumentParser\nprint(\"Hello\")".write(
        ...
        try "import TestingFramework\nimport TestPackage".write(
            to: testDir.appendingPathComponent("TestPackageTests.swift"), atomically: true, encoding: .utf8)
    ...
    private func createComplexPackage() throws -> URL {
        ...
        try "import ArgumentParser\npublic func hello() {}".write(
            to: libDir.appendingPathComponent("Library.swift"), atomically: true, encoding: .utf8)

        // Create app target
        let appDir = tempDir.appendingPathComponent("Sources/MyApp")
        try FileManager.default.createDirectory(at: appDir, withIntermediateDirectories: true)
        try "import SwiftUI\nimport MyLibrary\nhello()".write(
            to: appDir.appendingPathComponent("main.swift"), atomically: true, encoding: .utf8)
    ...
    private func createSimpleTestPackage() throws -> URL {
        ...
        try "print(\"Simple\")".write(
            to: sourceDir.appendingPathComponent("main.swift"), atomically: true, encoding: .utf8)
