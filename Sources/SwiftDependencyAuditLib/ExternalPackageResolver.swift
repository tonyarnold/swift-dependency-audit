...
public actor ExternalPackageResolver {
    ...
    public func resolveExternalPackages(for packageInfo: PackageInfo) async throws -> [ExternalPackage] {
        ...
        guard let checkoutsDir = checkoutsPath,
            FileManager.default.fileExists(atPath: checkoutsDir)
        else {
        ...
        for dependency in packageInfo.externalDependencies {
            if let externalPackage = try await resolveExternalPackage(
                dependency: dependency, checkoutsPath: checkoutsDir)
            {
    ...
    private func resolveExternalPackage(dependency: ExternalPackageDependency, checkoutsPath: String) async throws
        -> ExternalPackage?
    {
        // Check cache first
        ...
        let packageCheckoutPath = findPackageInCheckouts(
            packageName: dependency.packageName, checkoutsPath: checkoutsPath)
